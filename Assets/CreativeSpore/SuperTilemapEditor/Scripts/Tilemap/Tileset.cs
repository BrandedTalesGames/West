...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class Tileset : ScriptableObject
    {
        ...
        public void UpdateTilesetConfigFromAtlasImportSettings()
        {
            ...
            if (textureImporter.textureType == UnityEditor.TextureImporterType.Sprite && textureImporter.spriteImportMode == UnityEditor.SpriteImportMode.Multiple)
            {
                UnityEditor.U2D.Sprites.SpriteDataProviderFactories factory = new UnityEditor.U2D.Sprites.SpriteDataProviderFactories();
                factory.Init();

                UnityEditor.U2D.Sprites.ISpriteEditorDataProvider dataProvider = factory.GetSpriteEditorDataProviderFromObject(textureImporter);
                ...
                if (spriteRects.Length >= 2)
                {
                    if (m_tiles.Count == 0 || !UnityEditor.EditorUtility.DisplayDialog("Import Sprite Sheet?", "This texture atlas contains sliced sprites. Do you want to overwrite current tiles with these ones?", "Yes", "No"))
                    {
                        ...
                        {
                            Rect rUV = new Rect(Vector2.Scale(spriteRect.rect.position, AtlasTexture.texelSize), Vector2.Scale(spriteRect.rect.size, AtlasTexture.texelSize));
                            ...
                            if (tiles.Count >= 2)
                            {
                                if (tiles[tiles.Count - 2].uv.y != tiles[tiles.Count - 1].uv.y)
                                {
                                    if (m_tilesetHeight == 1)
                                    {
                                        ...
                                        SlicePadding.y = spriteRects[tiles.Count - 2].rect.y - spriteRects[tiles.Count - 1].rect.yMax;
                                    }
                                    ++m_tilesetHeight;
                                }
                            }
                        ...
                        if (m_tiles != null && m_tiles.Count > 0 && UnityEditor.EditorUtility.DisplayDialog("Keep previous tile properties?", "Keeping previous tile properties will copy the collider and parameters of previous tiles", "Yes", "No"))
                        {
                            for (int i = 0; i < m_tiles.Count && i < tiles.Count; ++i)
                            {
                                tiles[i].collData = m_tiles[i].collData;
